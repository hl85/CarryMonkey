...
export class CarryMonkeyLogger {
    ...
    if (!isFeatureEnabled("storeCompliant")) {
      this.addTransport((logObj) => {
        ...
        if (
          ...
          (meta.logLevelId as number) >= LogLevel.ERROR
        ) {
          // 错误日志特殊处理
          console.group(
            `🚨 ${meta.logLevelName} - ${logObj.component || "Unknown"}`,
          );
  ...
  private async cleanupOldLogs(): Promise<void> {
    try {
      ...
      const logKeys = Object.keys(storage).filter((key) =>
        key.startsWith("log_"),
      );

      if (logKeys.length > 1000) {
        // 删除最旧的日志
        const keysToRemove = logKeys.sort().slice(0, logKeys.length - 1000);
  ...
  createComponentLogger(
    component: string,
    additionalContext?: Partial<CarryMonkeyLogObj>,
  ): Logger<CarryMonkeyLogObj> {
  ...
  async getLogStats(): Promise<{
    ...
    try {
      const storage = await chrome.storage.local.get();
      const logs = Object.values(storage).filter(
        (item: unknown) =>
          item &&
          typeof item === "object" &&
          (item as Record<string, unknown>)._meta,
      );
      ...
      const lastLog = logs.sort((a: unknown, b: unknown) => {
        const aObj = a as Record<string, unknown>;
        const bObj = b as Record<string, unknown>;
        return (
          new Date((bObj.stored as string) || 0).getTime() -
          new Date((aObj.stored as string) || 0).getTime()
        );
...
export const createComponentLogger = (
  component: string,
  context?: Partial<CarryMonkeyLogObj>,
