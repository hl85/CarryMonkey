...
export class UnifiedInjectionEngine {
  ...
  static async injectScript(script: UserScript, tabId: number): Promise<void> {
    ...
    if (isFeatureEnabled("storeCompliant")) {
      ...
      if (!validation.safe) {
        ...
        throw new Error(
          `Script contains non-compliant code: ${validation.issues.join(", ")}`,
        );
      }
    ...
    try {
      ...
      if (isFeatureEnabled("storeCompliant")) {
        ...
        try {
          ...
          engineLogger.warn(
            "Compliant injection failed, trying compatibility fallback",
            {
              scriptId: script.id,
      ...
      if (
        ...
        !isFeatureEnabled("legacyInjection") &&
        isFeatureEnabled("evalFallback")
      ) {
   ...
   * 批量注入脚本
   */
  static async injectMultipleScripts(
    scripts: UserScript[],
    tabId: number,
  ): Promise<void> {
    const startTime = performance.now();
  ...
  static async healthCheck(): Promise<{
    ...
    try {
      ...
      if (
        isFeatureEnabled("userScriptsAPI") &&
