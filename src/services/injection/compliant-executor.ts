...
export class CompliantScriptExecutor {
  ...
  static createCompliantExecutor() {
    return function (scriptContent: string, scriptName: string) {
      ...
      try {
        ...
        if (window.trustedTypes && window.trustedTypes.createPolicy) {
          try {
            const policy = window.trustedTypes.createPolicy(
              "carrymonkey-compliant",
              {
                createScript: (input: string) => input,
              },
            );
            scriptElement.textContent = policy.createScript(
              scriptContent,
            ) as string;
          } catch (error) {
            ...
            console.error(
              `🐒[CarryMonkey Compliant] Trusted Types policy creation failed: ${scriptName}`,
              error,
            );
            return;
          }
        ...
        // Note: This log is embedded in the injected script
        console.log(
          `🐒[CarryMonkey Compliant] Script injected successfully: ${scriptName}`,
        );
      } catch (error) {
        // Note: This error is embedded in the injected script
        console.error(
          `🐒[CarryMonkey Compliant] Script injection failed: ${scriptName}`,
          error,
        );
      }
    };
  ...
  static createReadOnlyExecutor() {
    return function (scriptContent: string, scriptName: string) {
      ...
      console.log(
        `🐒[CarryMonkey Read-Only] Script registered but not executed: ${scriptName}`,
      );
      console.log(
        `🐒[CarryMonkey Read-Only] Script length: ${scriptContent.length} characters`,
      );
      ...
      try {
        ...
        // Note: This error is embedded in the injected script
        console.error(
          `🐒[CarryMonkey Read-Only] Script analysis failed: ${scriptName}`,
          error,
        );
      }
    };
   ...
   * 验证脚本内容是否安全
   */
  static validateScriptContent(script: UserScript): {
    safe: boolean;
    issues: string[];
  } {
    const issues: string[] = [];
