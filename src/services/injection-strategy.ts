...
export class InjectionStrategySelector {
  ...
  static async selectStrategy(script: UserScript): Promise<InjectionStrategy> {
    ...
    if (needsEarlyExecution && hasSpecialAPIs && supportsUserScriptsAPI) {
      ...
      selectedStrategy = {
        ...
        timing: this.convertRunAtTiming(runAt),
        reason:
          "UserScripts API available, using for better isolation and performance",
      };
      ...
      selectedStrategy = {
        ...
        timing: this.convertRunAtTiming(runAt),
        reason:
          "Isolation required but UserScripts API not available, using content script",
      };
      ...
      selectedStrategy = {
        ...
        timing: this.convertRunAtTiming(runAt),
        reason:
          "Simple script without special requirements, using direct injection",
      };
   ...
   */
  private static convertRunAtTiming(
    runAt: string,
   ...
   * 评估脚本复杂度
   */
  static evaluateScriptComplexity(
    script: UserScript,
      ...
      factors: {
        baseScore:
          strategy.method === "userscripts-dynamic"
            ? 100
            : strategy.method === "userscripts-api"
              ? 80
              : 60,
        timingMatch:
          this.convertRunAtTiming(requestedTiming) === strategy.timing,
