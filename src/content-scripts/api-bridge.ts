...
window.GM_getResourceText = async (resourceName: string): Promise<string> => {
  const result = await sendMessage("GM_getResourceText", {
    scriptId: window.currentScriptId,
    resourceName,
  });
...
window.GM_getResourceURL = async (resourceName: string): Promise<string> => {
  const result = await sendMessage("GM_getResourceURL", {
    scriptId: window.currentScriptId,
    resourceName,
  });
  return result as string;
